...
{
    public partial class Главное_меню_учебного_отдела_эксперта : Form
    {
        ...
        public static string accessin = "0";
        ...
        public Главное_меню_учебного_отдела_эксперта()
        {
            InitializeComponent();
        ...
        private void Главное_меню_учебного_отдела_эксперта_Load(object sender, EventArgs e)
        {
        ...
        private void расчетУровняСформированностиКомпетенцийСтудентовToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            finally
            {
                ...
                sql = @"SELECT Г.Название_направления, А.ФИО_студента, В.Название_компетенции_студента, Б.Процент_сформированности_компетенции 
                    FROM Студент А, Уровень_сформированности_компетенций_студентов Б, Компетенции_студентов В, Направления_обучения Г
                    WHERE А.Код_студента = Б.Код_студента
                    AND Б.Код_компетенции_студента = В.Код_компетенции_студента
                    AND Г.Код_направления_обучения = В.Код_направления_обучения
                    AND Б.Код_направления_обучения = Г.Код_направления_обучения
                    ORDER BY Г.Название_направления
                        ...
                        switch (cl)
                        {
                            ...
                            case 5:
                                ...
                                insert = new SqlCommand("INSERT INTO Баллы_за_учебную_деятельность(Код_записи, Код_студента, Код_дисциплины, Код_компетенции_студента, " +
                                    "Код_КОСа, Код_преподавателя, Количество_набранных_баллов, Дата_записи) " +
